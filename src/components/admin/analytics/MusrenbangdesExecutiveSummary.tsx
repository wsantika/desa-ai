...
export function MusrenbangdesExecutiveSummary({
  ...
  return (
    <div
      ...
      className="printable-musrenbangdes-area flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900"
    >
      {/* Official Header Kop */}
      <div className="border-b-2 border-slate-900 pb-4 text-center dark:border-slate-700">
        <div className="flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-700 dark:text-blue-400">
          <Building2 className="h-4 w-4" />
          <span>Pemerintah Kabupaten Gianyar • Kecamatan Gianyar</span>
        </div>
        <h2 className="mt-1 mb-0 font-sans text-lg sm:text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
          PEMERINTAH DESA TEGAL TUGU
        </h2>
        <p className="mt-0.5 mb-2 text-xs font-medium text-slate-500 dark:text-slate-400">
          Jalan Raya Tegal Tugu No. 1, Kecamatan Gianyar, Kabupaten Gianyar,
          Bali 80515
        </p>
        <div className="mx-auto inline-block rounded-md bg-blue-50 px-3 py-1 text-xs font-bold text-blue-900 dark:bg-blue-950/60 dark:text-blue-200">
          LEMBAR RINGKASAN EKSEKUTIF ANALITIK MUSRENBANGDES (RKPDES 2027)
        </div>
      </div>

      {/* Meta Evaluation Info */}
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3 rounded-lg border border-slate-200 bg-slate-50 p-3.5 text-xs dark:border-slate-800 dark:bg-slate-900/50">
        <div>
          <span className="text-slate-500 dark:text-slate-400">
            Periode Data:
          </span>
          <p className="m-0 font-semibold text-slate-900 dark:text-slate-200">
        ...
        <div>
          ...
          <p className="m-0 font-semibold text-blue-700 dark:text-blue-400">
      ...
      <div className="mt-4 rounded-lg border-l-4 border-blue-600 bg-blue-50/60 p-3.5 text-xs text-blue-950 dark:border-blue-400 dark:bg-blue-950/30 dark:text-blue-200">
        ...
        <div className="space-y-3">
          {summary.recommendations.map((rec, index) => {
            ...
            return (
              <div
                key={rec.id}
                className="rounded-lg border border-slate-200 p-4 text-xs transition-shadow hover:shadow-xs dark:border-slate-800"
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-[10px] font-bold text-white dark:bg-blue-700">
                      {index + 1}
                    </span>
                    <span className="font-bold text-slate-900 dark:text-slate-200">
                      {rec.pillar}
                    </span>
                    <span className="flex items-center gap-1 rounded-sm bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                ...
                <h4 className="mt-2 mb-1 text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100">
                  {rec.title}
                </h4>
        ...
        <div className="flex flex-col sm:flex-row items-end justify-between gap-6 text-xs">
          ...
          <div className="w-full sm:w-56 text-center">
            ...
            <div className="my-6 flex justify-center">
              <div className="flex h-14 w-28 items-center justify-center rounded-sm border border-dashed border-blue-600/40 text-[10px] font-semibold text-blue-700/60 dark:border-blue-400/40 dark:text-blue-300/60">
                [ Tanda Tangan & Cap ]
              </div>
            </div>
            <p className="m-0 font-bold text-slate-900 underline dark:text-slate-100">
