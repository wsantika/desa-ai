...
describe('Issue #11: Citizen Platform Layout & PWA Configuration', () => {
  ...
  it('validates manifest.webmanifest structure, branding, and standalone display', () => {
    assert.ok(
      ...
      'manifest.webmanifest must exist in public directory',
    )
    ...
    assert.equal(
      manifest.name,
      'Desa Tegal Tugu — Platform Layanan Warga Cerdas',
    )
    ...
    assert.ok(
      ...
      'manifest must have an icons array',
    )
    assert.ok(
      manifest.icons.length >= 2,
      'manifest must have at least 2 icon sizes',
    )
  ...
  it('validates PWA icons byte size and vector SVG integrity', () => {
    ...
    assert.ok(
      ...
      'icon.svg must contain valid SVG tag',
    )
    assert.ok(
      svgContent.includes('viewBox="0 0 512 512"'),
      'icon.svg must have 512x512 viewBox',
    )

    const png192Stat = fs.statSync(png192Path)
    assert.ok(
      png192Stat.size > 500,
      'icon-192.png must have valid non-empty byte size',
    )

    const png512Stat = fs.statSync(png512Path)
    assert.ok(
      png512Stat.size > 1000,
      'icon-512.png must have valid non-empty byte size',
    )
  })
  ...
  it('validates Service Worker caching and event listener setup in public/sw.js', () => {
    ...
    assert.ok(
      swContent.includes('desa-mandara-v1'),
      'SW must use desa-mandara cache version',
    )
    assert.ok(
  ...
  it('validates citizen platform routes are registered in route tree', () => {
    ...
    assert.ok(
      content.includes("'/asisten'"),
      'asisten route must be registered',
    )
    assert.ok(
      content.includes("'/layanan'"),
      'layanan route must be registered',
    )
    assert.ok(
      content.includes("'/pengaduan'"),
      'pengaduan route must be registered',
    )
    assert.ok(
      content.includes("'/layanan/pengajuan'"),
      'layanan/pengajuan redirect route must be registered',
    )
    assert.ok(
      content.includes("'/pengaduan/baru'"),
      'pengaduan/baru redirect route must be registered',
    )
    assert.ok(
      content.includes("'/pelacakan'"),
      'pelacakan redirect route must be registered',
    )
  })
  ...
  it('validates mobile safe-area insets and bottom navigation layout integration', () => {
    ...
    assert.ok(
      rootContent.includes('<BottomNav />'),
      '__root.tsx must include BottomNav component',
    )
    assert.ok(
      rootContent.includes('manifest.webmanifest'),
      '__root.tsx must link manifest',
    )
    assert.ok(
      rootContent.includes('viewport-fit=cover'),
      '__root.tsx must include viewport-fit=cover for mobile notches',
    )
    ...
    assert.ok(
      ...
      'BottomNav must account for safe-area insets',
    )
    assert.ok(
      bottomNavContent.includes('sm:hidden'),
      'BottomNav must be mobile-first and hidden on desktop',
    )
  })
})
